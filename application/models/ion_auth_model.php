...
class Ion_auth_model extends CI_Model
{
	...
	public function fb_user_check($fb_userid = 0)
	{
	    if (intval($fb_userid) == 0)
	    {
	        return FALSE;
	    }
	    
	    return $this->db->where('fb_userid', $fb_userid)
	        ->count_all_results($this->tables['meta']) > 0;
	}
	...
	public function update_last_login($id)
	{
	    ...
	    $this->db->update($this->tables['users'], array('last_login' => date("Y-m-d H:i:s", now())), array('id' => $id));
    ...
    public function login_fb_user($fb_userid)
    {
        log_message('debug', '* ION Model: login_fb_user IN');
        ...
        // Find FB User ID in table meta
        // Get the user_id to get the user row later
        $fb_query = $this->db->select('id, user_id, fb_userid')
            ->where('fb_userid', $fb_userid)
            ->limit(1)
            ->get($this->tables['meta']);
            
        if ($fb_query->num_rows() != 1)
        {
            return FALSE;
        }
        
        $fb_user = $fb_query->row();
    
        $query = $this->db->select($this->identity_column.', id, group_id')
                  ->where('id', $fb_user->user_id)
                  ->limit(1)
                  ->get($this->tables['users']);
