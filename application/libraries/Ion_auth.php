...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ci->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			$email_template = $this->ci->config->item('email_templates', 'ion_auth')
			. $this->ci->config->item('language_abbr') . '/'
			. $this->ci->config->item('email_forgot_password', 'ion_auth');

			$message = $this->ci->load->view($email_template, $data, true);
			$this->ci->email->clear();
			...
			$this->ci->email->subject($this->ci->config->item('site_title', 'ion_auth') . ' - ' . _('Forgotten Password Verification'));
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			$email_template = $this->ci->config->item('email_templates', 'ion_auth')
			. $this->ci->config->item('language_abbr') . '/'
			. $this->ci->config->item('email_forgot_password_complete', 'ion_auth');
			...
			$this->ci->email->subject($this->ci->config->item('site_title', 'ion_auth') . ' -' . _('New Password'));
			$this->ci->email->message($message);
	...
	public function register($username, $password, $email, $additional_data, $group_name = false) //need to test email activation
	{
		...
		else
		{
			...
			$this->ci->email->subject($this->ci->config->item('site_title', 'ion_auth') . ' - ' . _('Account Activation'));
			$this->ci->email->message($message);
	...
	public function logout()
	{
		...
		$this->set_message(_('Logout_successful'));
		return TRUE;
	}
