...
class MY_Pagination extends CI_Pagination
{
    ...
    function _set_pagination_offset ()
    {
        ...
        if (strstr($CI->uri->uri_string(), $this->pagination_selector)) {
            ...
            if ($base_url = $this->_check_language($CI->uri->uri_string()))
            {
            ...
            log_message('debug', '* Pagination URI: '.$uri);
            log_message('debug', '* Pagination pos: '.$pos);
            log_message('debug', '* Pagination Base URL: '.$this->base_url);
            log_message('debug', '* Pagination config base_url: '.config_item('base_url'));
        }
        else 
        { // Pagination selector was not found in URI string. So offset is 0
            ...
            log_message('debug', '* Pagination no page selector: '.$CI->uri->uri_string());
            
            // Check language without page selector
            if ($base_url = $this->_check_language($CI->uri->uri_string(), TRUE))
            {
                $this->base_url = $base_url;
            }
        }
    ...
    private function _check_language($uri, $no_selector = FALSE)
    {
        /** 
         * Checks if the URI starts with a language code to strip it out of the pagination links (to avoid duplicate language code)
         * If no page selector is given (case of first page with no /Page/) adds pagination selector
         */
        ...
        $check_language_uri = substr($uri, 0, 3);

        if (substr($check_language_uri, -1, 1) == '/')
        {
            $check_language = substr($check_language_uri, 0, 2);

            if (array_key_exists($check_language, $CI->config->item('lang_uri_abbr')))
            {
                //log_message('debug', '* Pagination check_language in array: '.$check_language);
                $base_url = config_item('base_url') . $this->index_page . substr($uri, 3, $pos + strlen($this->pagination_selector)-3);
                
                if ($no_selector)
                {
                    $base_url = config_item('base_url') . $this->index_page . substr($uri, 3) . '/' . $this->pagination_selector;
                }
                //log_message('debug', '* Pagination URI in _check_language: '.$uri);
            }
        }
